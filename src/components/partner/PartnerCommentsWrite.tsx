...
import { getAuthId, postPartnerComment } from './Partner';
...
function PartnerCommentsWrite({ prevComment }: { prevComment: string }) {
  ...
  const { isLoading, data } = useQuery(queryKey, getAuthId);
  if (isLoading) {
    console.log('로딩중');
  }
  console.log('왜안떠', data); // undifined
  ...
  const initialValue = {
    ...
    writerId: 'a98635da-3718-4503-babb-69cec5241c54',
  ...
  return (
    <>
      <Form onSubmit={handleSubmitBtnClick}>
        ...
        <Input type="text" name="content" placeholder="content" value={inputContent} onChange={handleCommentChange} />
